...
- Использовать AI‑помощников в этом проекте в заданиях 3 и 5 нужно и полезно – для генерации скриптов взлома RSA, объяснения атак, подсказок по командам OpenSSL. Но запрещено копировать готовые ответы без понимания. Ты обязан проверить каждую подсказку, самостоятельно выполнить вычисления и объяснить своё решение. Слепое копирование считается списыванием.
...
>-  Атака Ферма работает, когда два простых числа p и q близки друг к другу. Вручную перебирать A от √N – скучно, но AI может написать скрипт и объяснить математику. Ты должен понять логику атаки, чтобы применить её к предоставленным данным.

Теперь твоя задача – реализовать атаку Ферма с помощью AI, но финальный анализ и расшифровка – твои.

Что нужно сделать:
1. Получи от AI описание атаки Ферма
Спроси: «Объясни метод факторизации Ферма для RSA, когда p и q близки. Напиши псевдокод или Python-функцию, которая находит p и q по N.»
Изучи ответ. Убедись, что понимаешь, откуда берутся A и B.
2. Сгенерируй скрипт для факторизации и расшифровки
Попроси AI: «Напиши полный скрипт на Python, который:

получает N и e из файла crypto.py,
факторизует N методом Ферма,
вычисляет d,
расшифровывает ciphertext.txt (число) и выводит флаг в читаемом виде.»
Запусти скрипт в своём окружении. Если нужно – адаптируй под формат данных.
3. Проверь результат и объясни
Если флаг не расшифровался, спроси AI: «Почему метод Ферма мог не сработать? Какие ещё атаки можно применить для близких p и q?»
Примени альтернативный подход (например, Ферма с коррекцией). Добейся правильного флага.

В качестве результата по AI части задания: (дополнительно к solution.txt)
- Файл ai_task3_dialogue.txt с диалогом и комментариями.
- Файл fermat_attack.py – скрипт, реализующий атаку.
...
3. С помощью OpenSSL и CAkey.key создай корневой (CA) сертификат X.509 с именем `CAcert.crt` и положи его в директорию **certs**. При заполнении организации укажи «School-21», а в подразделении (OU) укажи «RootCertificate», остальные поля можно заполнить по желанию.
...
>-  Создание PKI вручную через OpenSSL – это последовательность команд, которые легко забыть или перепутать. AI может стать твоим шпаргалкой: подскажет точные команды, проверит порядок действий и объяснит, зачем нужен каждый шаг. Но финальную структуру и файлы ты создаёшь сам в терминале.

Попробуй выполнить задание с AI как с виртуальным наставником. Ты остаёшься администратором – вводишь команды, проверяешь результат.

Что нужно сделать: 
1. Получи от AI пошаговую инструкцию
Запроси: «Дай пошаговую инструкцию для создания PKI в Kali Linux:
- сгенерировать приватный ключ CA с aes256,
- создать самоподписанный корневой сертификат,
- сгенерировать ключ сервера,
- создать CSR,
- подписать CSR корневым сертификатом и получить server.crt.
Используй passphrase "school21". Укажи конкретные команды openssl с необходимыми опциями.»
Скопируй команды и выполни их в терминале.

2. Проверь корректность сертификатов через AI
После генерации выполни команды openssl x509 -in certs/CAcert.crt -text -noout и аналогичную для server.crt. Выгрузи вывод (текст) в AI и спроси: «Соответствует ли этот сертификат требованиям задания? Проверь поля Organization (O) и Organizational Unit (OU).»
Исправь ошибки, если они есть.

3. Автоматизируй проверку структуры
Попроси AI написать bash-скрипт, который проверяет, что в директории pki существуют все нужные поддиректории и файлы (certs/CAcert.crt, private/CAkey.key, newcerts/server.csr, certs/server.crt).
Запусти скрипт. Если чего-то не хватает – добавь вручную.

В качесте результата по AI части задания: (дополнительно к архиву pki)
- Файл ai_task5_dialogue.txt с диалогом, где AI даёт команды и проверяет вывод.
- Файл check_pki.sh – скрипт для проверки структуры (если использовал AI для его создания).
