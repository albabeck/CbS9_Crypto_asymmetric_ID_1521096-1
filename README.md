# Асимметричная криптография - Ассимметрия жизни
Аннотация: В данном проекте ты познакомишься с ассиметричным шифрованием, алгоритмом RSA, алгоритмом Диффи Хеллмана, атаками на алгоритмы ассиметричного шифрования, а также с инструментами и инфраструктурой открытых ключей, которые работают за счет ассиметричного шифрования

## Содержание
1. [Chapter I](#chapter-i) \
   1.1. [Рекомендации к проекту](#рекомендации-к-проекту)
2. [Chapter II](#chapter-ii) \
   2.1. [Ассиметричное шифрование](#ассиметричное-шифрование)
3. [Chapter III](#chapter-iii) \
   ...
   3.3. [Задание 3. Атаки на алгоритмы ассиметричного шифрования](#задание-3-атаки-на-алгоритмы-ассиметричного-шифрования) \
   3.4. [Задание 4. Инструменты работы с ассиметричным шифрованием](#задание-4-инструменты-работы-с-ассиметричным-шифрованием) \
...
Как ты уже знаешь, ассиметричное шифрование отличается от симметричного использованием двух ключей - закрытого и открытого. При этом наличие открытого (публичного) ключа позволяет осуществлять его передачу по открытым каналам, не опасаясь, что с его помощью кто-то сможет расшифровать перехваченное сообщение

Ассиметричное шифрование намного популярнее симметричного, когда речь заходит о передаче данных по сети и при установлении защищенных соединений (протоколы TLS и SSL используют именно ассиметричный алгоритм на стадии handshake). 

Наиболее популярным алгоритмом ассимметричного шифрования является RSA. Чтобы погрузиться в него более предметно и изучить атаки на этот алгоритм, рекомендую прочитать книгу "Thirty Years of Attacks on the RSA Cryptosystem".
...
Когда мы имеем дело с действительными числами и вычисляем a - b, мы также делаем
a + (-b). Но отрицательные числа не входят в наш диапазон: 0 ≤ x < n, где
n — наш модуль. Если 0 ≤ b < n, то -b = n - b, и вместо 
вычитания, мы можем добавить (-b). 

#### Мультипликативная инверсия, НОД и функция Эйлера 
Наибольший общий делитель (НОД) 
> НОД(a, b) a, b ∈ ℤ — наибольшее целое число, делящее 
> и a, и b. 
...
Расчет НОД с использованием этой функции выполняется за время O(log(N)), где
N — размер числа (в битах). Этот факт чрезвычайно важен, поскольку 
RSA использует огромные числа, и часто приходится иметь очень ограниченное количество времени для решения задач с RSA.
...
В последнем выражении мы получили 1, потому что обратного для 6 mod 12 не существует, посколько НОД(6,12) = 6, а должен быть 1

##### Обратное по модулю существует только в случае, если НОД(a, b) = 1.

Что такое функция Эйлера? 
> Функция Эйлера, обозначаемая Φ(n), показывает, сколько чисел меньше
> n, при этом взаимно простых с n. 

Что значит взаимно простой? 
> Два числа являются взаимно простыми, если у них нет общих делителей, поэтому их 
> НОД равен 1

В RSA нас в основном будет интересовать Φ(n), где n — простое число, при этом существует
n - 1 целых чисел меньше n, у которых нет общего множителя с n. 
...
1. Выберите два больших простых числа p и q, выбор простых чисел важен 
2. Вычислить N = p⋅q и Φ = (p – 1)(q – 1) 
3. Выберите экспоненту e такой, что НОД(e, Φ) = 1. Обычно это 65537.
4. Рассчитайте экспоненту расшифровки d = e<sup>-1</sup> (mod Φ). 
5. Ваш открытый ключ это (N, e), который вы можете передавать по открытым каналам
6. d — ваш личный ключ, никогда нельзя передавать его (или любой из p, q и Φ)!

Чтобы зашифровать сообщение **p**, для начала нужно перевести **p** в число. Затем получаем зашифрованный текст **c**
>c = p<sup>e</sup> (mod n).


Чтобы расшифовать зашифрованный текст **c**, имея ключ расшифровки **d**
> p = c<sup>d</sup> (mod n)

Где **p** - исходный текст. Осталось только конвертировать его обратно в строку

Твое задание:
Имея следующую информацию, найди изначальный текст:
...
Алгоритм Диффи-Хеллмана решает одну из важнейших проблем криптографии - **распределение ключей**. Эта криптосистема была изобретена раньше алгоритма шифрования RSA, но непосредственно для шифрования с открытым ключем или, например, для подписания сертификатов, она не используется. Ее основная задача - обмен ключами.
...
Изучи скрипт "dh.py" с алгоритмом Диффи-Хеллмана на языке python и найди в нем логическую ошибку реализации.
В качестве ответа напиши в файл "dh_answer.txt" через **";"** следующее:
...
### Задание 3. Атаки на алгоритмы ассиметричного шифрования
...
>- Замазка - самый страшный инструмент, с которого все и началось, но притворяется добрым что, впрочем, ему удаётся. Не женат.
>- Ластик - старый и потертый жизнью. Любит говорить всем правду в глаза, отчего и страдает. Не женат.
>- Ручка - новенькая в этой компании. Подруга ластика. Жадина, ради чернил готова на всё. Не замужем.
...
>
>Замазка внезапно открывает колпачок и пачкает весь пенал, вместе со всем содержимым. Девочка, испугавшись замазки, а также в силу невозможности отмыть всех остальных, выкидывает их в мусорку, тем самым даруя свободу. Этот план не устраивал Карандаша, но он был влюблен в Ручку, поэтому готов был идти с ней до конца

На алгоритм RSA, как классический пример ассиметричного алгоритма шифрования, существует множество атак. Однако, все они связаны не с самой "логикой" работы алгоритма, а с неверным выбором исходных чисел. Т.е. при использовании маленьких чисел в качестве экспоненты/экспоненты расшифровки, появляется возможность для атак.

В числе тех, которые стоит изучить, можно выделить: атака Копперсмита, атака Хастада и атака Винера.
...
Так же через опцию **-b** можно задавать другую длину ключа
...
Ознакомиться со списком всех команд можно на официальном сайте в разделе документации:

https://docs.openssl.org/3.0/man1/

Твое задание:

Поочередно выполни следующие действия в Kali Linux:

1) Создай пару ключей (приватный/публичный) с помощью ssh-keygen. Алгоритм: RSA, длина ключа: 2048
2) Конвертируй приватный ключ openssh в формат PEM с помощью ssh-keygen
...
Существует множество архитектур PKI, в которых множество удостоверяющих центров. О них (и в целом о том, зачем нужен PKI) можно прочитать, например, здесь: 

https://ru.wikibooks.org/wiki/%D0%92%D0%B2%D0%B5%D0%B4%D0%B5%D0%BD%D0%B8%D0%B5_%D0%B2_PKI
...
Твое задание заключается в создании своей PKI. Тебе нужно создать сертификаты CA, root и сертификат сервера. Для этого выполни следующие шаги в терминале Kali Linux:
...
Получившуюся директорию **pki** заархивируй и залей на гит. Поздравляю, ты настроил локальную инфраструктуру открытых ключей.
